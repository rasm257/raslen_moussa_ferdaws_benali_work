...
<body>
    ...
    <?php
        ...
        if($_SERVER['REQUEST_METHOD']==='POST'){
            $query = "UPDATE student SET id = :id, nom = :nom, date_naissance=:date_naissance,section_id = :section_id WHERE id = ".$_POST['c_student_id'].";";
            $stmt = $pdo->prepare($query);
            $stmt->bindParam(':id',$_POST['n_student_id'],PDO::PARAM_INT);
            $stmt->bindParam(':nom',$_POST['n_student_name'],PDO::PARAM_STR);
            $stmt->bindParam(':date_naissance',$_POST['n_student_date_naissance'],PDO::PARAM_STR);
            $stmt->bindParam(':section_id',$_POST['n_student_section_id'],PDO::PARAM_INT);
            $stmt->execute();
            echo "Student Data changed successfully!";
        }
    ?>
    </h1>
    <script>
